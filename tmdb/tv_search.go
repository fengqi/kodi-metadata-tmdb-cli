...
func (t *Tmdb) SearchShows(chsTitle, engTitle string, year int) (*SearchResults, error) {
	...
	for _, req := range searchComb {
		...
		if len(tvResp.Results) > 0 {
			selected := selectShowResult(chsTitle, engTitle, year, tvResp.Results)
			utils.Logger.DebugF("search tv: %s %d result count: %d, use: %v", chsTitle, year, len(tvResp.Results), selected)
			return selected, nil
		}
	}

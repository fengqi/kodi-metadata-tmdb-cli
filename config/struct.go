...
type FfmpegConfig struct {
	MaxWorker   int    `json:"max_worker"`   // 最大进程数，建议逻辑 CPU 个数
...
type CollectorConfig struct {
	...
	NfoField        NfoField `json:"nfo_field"`         // NFO 字段开关
	SkipFolders     []string `json:"skip_folders"`      // 跳过目录，可多个
	SkipKeywords    []string `json:"skip_keywords"`     // 跳过文件名关键字，可多个
	MoviesNfoMode   int      `json:"movies_nfo_mode"`   // 电影 NFO 模式：1 movie.nfo，2 <VideoFileName>.nfo（当前版本未启用）
	MoviesDir       []string `json:"movies_dir"`        // 电影根目录，可多个
	ShowsDir        []string `json:"shows_dir"`         // 剧集根目录，可多个
	MusicVideosDir  []string `json:"music_videos_dir"`  // 音乐视频根目录，可多个
}
